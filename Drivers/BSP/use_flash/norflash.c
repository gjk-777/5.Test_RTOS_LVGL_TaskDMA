...
void Test_w25q128_Flash_Run(void *pvParameters)
{
    ...
    printf("\r\n===================================\r\n");
    printf("       SFUD W25Q128 Full Test Start     \r\n");
    printf("===================================\r\n");

    /* 1. SFUD initialize */
    if (sfud_init() == SFUD_SUCCESS)
    {
        printf("[1] SFUD initialization successful.\r\n");
    }
    else
    {
        printf("[1] SFUD initialization failed!\r\n");
    }

    /* 2. Get flash device */
    sfud_flash *flash = sfud_get_device(SFUD_W25Q128_DEVICE_INDEX);
    if (flash == NULL)
    {
    ...
    /* 3. Test Read Status Register */
    printf("[3] Testing Read Status Register...\r\n");
    err = sfud_read_status(flash, &status);
    if (err == SFUD_SUCCESS)
    {
    ...
    /* 4. Test Erase */
    printf("[4] Testing Erase (Sector 0)...\r\n");
    if (sfud_erase(flash, test_addr, 4096) != SFUD_SUCCESS)
    {
    ...
    else
    {
        ...
        {
            printf("    Erase Verification Failed! Found non-0xFF data.\r\n");
            error_count++;
        }
    }

    /* 5. Test Write (Standard Write) */
    printf("[5] Testing Standard Write (256 Bytes)...\r\n");
    for (i = 0; i < 256; i++)
        write_buf[i] = i;

    if (sfud_write(flash, test_addr, 256, write_buf) != SFUD_SUCCESS)
    {
    ...
    /* 6. Test Read and Verify */
    printf("[6] Testing Read and Verify...\r\n");
    sfud_read(flash, test_addr, 256, read_buf);
    int verify_errors = 0;
    for (i = 0; i < 256; i++)
    {
        ...
        {
            printf("    Verify Error at index %d: Read 0x%02X, Expected 0x%02X\r\n", i, read_buf[i], write_buf[i]);
            verify_errors++;
            error_count++;
            if (verify_errors > 5)
            {
                printf("    ... too many errors, stopping verify.\r\n");
                break;
            }
        }
    }
    if (verify_errors == 0)
        printf("    Read & Verify Passed!\r\n");
    else
        printf("    Read & Verify Failed!\r\n");

    /* 7. Test Erase & Write (Auto Erase before Write) */
    printf("[7] Testing Erase & Write (sfud_erase_write)...\r\n");
    // Change data pattern
    for (i = 0; i < 256; i++)
        write_buf[i] = 0xFF - i;

    if (sfud_erase_write(flash, test_addr, 256, write_buf) != SFUD_SUCCESS)
    {
        printf("    Error: Erase & Write failed!\r\n");
        error_count++;
    }
    else
    {
        ...
        if (verify_errors == 0)
            printf("    Erase & Write Verified Passed!\r\n");
        else
            printf("    Erase & Write Verification Failed!\r\n");
    }

    /* 8. Summary */
    printf("\r\n===================================\r\n");
    if (error_count == 0)
        printf("       SFUD ALL Tests PASSED       \r\n");
    else
        printf("       SFUD Tests FAILED (%d errors)\r\n", error_count);
    printf("===================================\r\n");

    sfud_log_info("SFUD W25Q128 Full Test Completed");
}
